...
function merge_to_small_bag() {
    ...
    let merged_filename =
        school_year + "學年度第" + semester + "學期補考小袋封面";
    ...
    let tmp_doc = DocumentApp.openById(
        small_template.makeCopy("暫時的小袋", folder).getId()
    );
    ...
    for (i = 0; i < data.length; i++) {
        ...
        let students_table = tmp_table
            .getCell(1, 0)
            .setPaddingTop(0)
            .setPaddingBottom(0)
            .appendTable(students_list);
        ...
        for (j = 0; j < students_table.getNumRows(); j++) {
            for (k = 0; k < students_table.getRow(j).getNumCells(); k++) {
                let cell_style = {};
                cell_style[DocumentApp.Attribute.HORIZONTAL_ALIGNMENT] =
        ...
        if (i % BATCH_SIZE == BATCH_SIZE - 1) {
            ...
            let pdf_name =
                merged_filename +
                "_" +
                (1 + BATCH_SIZE * Math.floor(i / BATCH_SIZE)).toLocaleString(
                    "en-US",
                    {
                ...
                "-" +
                (BATCH_SIZE * (1 + Math.floor(i / BATCH_SIZE))).toLocaleString(
                    "en-US",
                    {
                        minimumIntegerDigits: number_of_digits,
                        useGrouping: false,
    ...
    if (data.length % BATCH_SIZE != 0) {
        ...
        pdf_name =
            ...
            "_" +
            (BATCH_SIZE *
                Math.floor(data.length / BATCH_SIZE).toLocaleString("en-US", {
                    minimumIntegerDigits: number_of_digits,
                    useGrouping: false,
                }) +
                1) +
